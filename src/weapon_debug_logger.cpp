...
void EnsureWeaponDebugLogOpen()
{
    ...
    _snprintf_s(
        ...
        sizeof(sessionLine),
        _TRUNCATE,
        "ts=%s map=%s event=weapon_debug_session status=ready file=\"%s\"",
        timestamp,
...
void LogAcceptedGlockPrimaryShot(CBasePlayer *pPlayer, const GlockAcceptedShotTelemetry &telemetry)
{
    ...
    _snprintf_s(
        ...
        sizeof(line),
        _TRUNCATE,
        "ts=%s map=%s player=\"%s\" entindex=%d userid=%d weapon=glock fire=primary experimental=%d tapfire=%d firstshot=%d spread=%.4f base=%.4f move_penalty=%.4f speed2d=%.1f maxspeed=%.1f grounded=%d ducking=%d delta_prev=%s clip=%d",
        timestamp,
        SanitizeLogValue(GetSafeMapName()).c_str(),
...
void LogRejectedGlockPrimaryHold(CBasePlayer *pPlayer, const GlockRejectedShotTelemetry &telemetry)
{
    ...
    _snprintf_s(
        ...
        sizeof(line),
        _TRUNCATE,
        "ts=%s map=%s player=\"%s\" entindex=%d userid=%d weapon=glock fire=primary reason=tapfire_hold_blocked speed2d=%.1f grounded=%d ducking=%d",
        timestamp,
        SanitizeLogValue(GetSafeMapName()).c_str(),
        GetSafePlayerName(pPlayer).c_str(),
        GetPlayerEntityIndex(pPlayer),
