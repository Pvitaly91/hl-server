...
function P([string]$l,[int]$n){$t=$l.Trim();if(-not $t){return $null};if($t.StartsWith('[')){$i=$t.IndexOf(']');if($i -gt 0){$t=$t.Substring($i+1).TrimStart()}};$m=[regex]::Matches($t,'(?<k>[A-Za-z0-9_]+)=(?:"(?<q>[^"]*)"|(?<v>\S+))');if($m.Count -eq 0){return $null};$h=[ordered]@{};foreach($x in $m){$h[$x.Groups['k'].Value]=if($x.Groups['q'].Success){$x.Groups['q'].Value}else{$x.Groups['v'].Value}};$type=(F $h 'type');if(-not $type){if((F $h 'event') -eq 'weapon_debug_session'){$type='session'}elseif(F $h 'reason'){$type='rejected'}elseif(F $h 'weapon'){$type='accepted'}else{$type='unknown'}};$w=(F $h 'weapon');if($w){$w=$w.ToLowerInvariant()};[pscustomobject]@{Line=$n;Type=$type.ToLowerInvariant();Timestamp=F $h 'ts';Map=F $h 'map';Game=F $h 'game';Status=F $h 'status';LogFile=F $h 'file';Weapon=$w;WeaponUnderTest=FF $h @('weapon_under_test','weaponUnderTest','sv_exp_weapon_under_test');ProfileName=F $h 'profile';GlockProfileName=FF $h @('glock_profile','glockProfile','sv_exp_glock_profile_name');Mp5ProfileName=FF $h @('mp5_profile','mp5Profile','sv_exp_mp5_profile_name');TargetProfileName=FF $h @('target_profile','targetProfile','sv_exp_glock_lab_target_profile_name');SessionTag=FF $h @('session_tag','sv_exp_session_tag');MatrixName=FF $h @('matrix_name','sv_exp_matrix_name');MatrixStep=FF $h @('matrix_step','sv_exp_matrix_step');CfgMode=B(F $h 'cfg_mode');CfgActiveProfile=FF $h @('cfg_active_profile','cfg_active');CfgLastSuccessfulProfile=FF $h @('cfg_last_successful_profile','cfg_last_success');CfgLastAction=FF $h @('cfg_last_action');CfgLastAppliedAt=FF $h @('cfg_last_applied_at');Reason=F $h 'reason';HitGroup=FF $h @('hitgroup','hitGroup');FirstShot=B(F $h 'firstshot');Spread=N(F $h 'spread');MovementPenalty=N(F $h 'move_penalty');BurstAddedSpread=N(FF $h @('burst_additional_spread','burstAddedSpread'));BurstIndex=I(FF $h @('burst_index','burstIndex'));HorizontalSpeed=N(F $h 'speed2d');MaxSpeed=N(F $h 'maxspeed');Grounded=B(F $h 'grounded');Ducking=B(F $h 'ducking');AppliedDamage=N(F $h 'applied_damage');Headshot=B(F $h 'headshot');HeadshotLethalApplied=B(F $h 'headshot_lethal_applied');VictimKind=FF $h @('victim_kind','victimKind');VictimClass=FF $h @('victim_class','victimClass');ArmorApplied=B(FF $h @('armor_applied','armorApplied'));HeadProtected=B(FF $h @('head_protected','headProtected'));DummyArmorBefore=N(FF $h @('dummy_armor_before','dummyArmorBefore'));Raw=$l}}
...
$exports=[ordered]@{};$exportEvents=if($Weapon -eq 'all'){$events}else{@($events|?{$_.Type -eq 'session' -or $_.Type -like 'dummy_*' -or (M $_)})};if($ExportJson -or $ExportCsv){$out=if([string]::IsNullOrWhiteSpace($OutputDir)){Get-WeaponDebugReportsRoot}else{Get-FullPath -Path $OutputDir};Ensure-Directory -Path $out;$stamp=Get-Date -Format 'yyyyMMdd-HHmmss';if($ExportJson){$j=Join-Path $out ('weapon-report-'+$stamp+'.json');$report|ConvertTo-Json -Depth 8|Set-Content -LiteralPath $j -Encoding UTF8;$exports.json=$j};if($ExportCsv){$c=Join-Path $out ('weapon-events-'+$stamp+'.csv');$exportEvents|Export-Csv -LiteralPath $c -NoTypeInformation -Encoding UTF8;$exports.csv=$c}}
Write-Host 'Weapon log analysis';Write-Host "  log path                 : $($log.FullName)";Write-Host "  weapon filter            : $Weapon";if($session){Write-Host "  session                  : ts=$($session.Timestamp) map=$($session.Map) game=$($session.Game) status=$($session.Status)";if($effectiveWeapon){Write-Host "  weapon under test        : $effectiveWeapon"};if($glockProfile){Write-Host "  glock profile            : $glockProfile"};if($mp5Profile){Write-Host "  mp5 profile              : $mp5Profile"};Write-Host "  target profile           : $(if($targetProfile){$targetProfile}else{'n/a'})"}else{Write-Host '  session                  : missing'};Write-Host "  accepted shots           : $($accepted.Count)";Write-Host "  rejected shots           : $($rejected.Count)";Write-Host "  hit events               : $($hits.Count)";Write-Host "  kill events              : $($kills.Count)";Write-Host "  headshot hits            : $($headshotHits.Count)";Write-Host "  headshot kills           : $($headshotKills.Count)";Write-Host "  target spawns            : $($dummySpawns.Count)";Write-Host "  target clears            : $($dummyClears.Count)";Write-Host "  target respawns          : $($dummyRespawns.Count)";if($dummyRepositions.Count -gt 0){Write-Host "  target repositions       : $($dummyRepositions.Count)"};Write-Host "  target hits              : $($dummyHits.Count)";Write-Host "  target kills             : $($dummyKills.Count)";Write-Host "  target hs hits           : $($dummyHeadshotHits.Count)";Write-Host "  target hs kills          : $($dummyHeadshotKills.Count)";Write-Host "  armored target hits      : $($armoredDummyHits.Count)";Write-Host "  protected hs hits        : $($protectedDummyHeadshotHits.Count)";Write-Host "  protected hs kills       : $($protectedDummyHeadshotKills.Count)";Write-Host "  target lethal hs evid.   : $($dummyLethal.Count)";Write-Host "  accepted move_penalty>0  : $($movePenaltyAccepted.Count)";if($effectiveWeapon -eq 'mp5' -or $Weapon -eq 'mp5'){Write-Host "  burst growth evidence    : $($burstGrowthAccepted.Count)"};if($effectiveWeapon -ne 'mp5'){Write-Host "  first-shot accepted      : $($firstShotAccepted.Count)"};Write-Host "  applied dmg min/avg/max  : $(if($appliedStats){('{0:F4} / {1:F4} / {2:F4}' -f $appliedStats.Min,$appliedStats.Average,$appliedStats.Max)}else{'n/a / n/a / n/a'})";Write-Host "  hitgroups                : $(FHitgroups $report.stats.hitgroupCounts)";Write-Host "  spread min/avg/max       : $(if($spreadStats){('{0:F4} / {1:F4} / {2:F4}' -f $spreadStats.Min,$spreadStats.Average,$spreadStats.Max)}else{'n/a / n/a / n/a'})";Write-Host "  move penalty min/avg/max : $(if($moveStats){('{0:F4} / {1:F4} / {2:F4}' -f $moveStats.Min,$moveStats.Average,$moveStats.Max)}else{'n/a / n/a / n/a'})";if($effectiveWeapon -eq 'mp5' -or $Weapon -eq 'mp5'){Write-Host "  burst add min/avg/max    : $(if($burstStats){('{0:F4} / {1:F4} / {2:F4}' -f $burstStats.Min,$burstStats.Average,$burstStats.Max)}else{'n/a / n/a / n/a'})"};Write-Host "  speed2d min/avg/max      : $(if($speedStats){('{0:F1} / {1:F1} / {2:F1}' -f $speedStats.Min,$speedStats.Average,$speedStats.Max)}else{'n/a / n/a / n/a'})";Write-Host '';Write-Host 'Observations';if($obs.Count -eq 0){Write-Host '  - No notable observations were derived from the parsed telemetry.'}else{foreach($o in $obs){Write-Host "  - $o"}};Write-Host '';Write-Host 'Warnings';if($warnings.Count -eq 0){Write-Host '  - none'}else{foreach($w in $warnings){Write-Host "  - $w"}};if($exports.Count -gt 0){Write-Host '';Write-Host 'Exports';foreach($k in $exports.Keys){Write-Host "  $k : $($exports[$k])"}};Write-Host '';Write-Host 'Evidence only: this summarizes logged server-authoritative telemetry. It does not validate client-side recoil feel, prediction quality, or exact PvP parity.';if($assert.Count -gt 0){Write-Host '';Write-Host 'Assertion failures';foreach($a in $assert){Write-Host "  - $a"};exit 2};if($PassThru){[pscustomobject]@{LogPath=$log.FullName;Report=$report;Exports=[pscustomobject]$exports}}
