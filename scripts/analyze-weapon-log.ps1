...
if($effectiveWeapon -eq 'glock' -or $effectiveWeapon -eq 'mp5' -or $Weapon -eq 'glock' -or $Weapon -eq 'mp5'){Write-Host "  pattern evidence         : $($patternAccepted.Count)"}
if($effectiveWeapon -eq 'glock' -or $effectiveWeapon -eq 'mp5' -or $Weapon -eq 'glock' -or $Weapon -eq 'mp5'){Write-Host "  pattern resets           : $($patternResetAccepted.Count)"}
...
if($effectiveWeapon -eq 'glock' -or $effectiveWeapon -eq 'mp5' -or $Weapon -eq 'glock' -or $Weapon -eq 'mp5'){Write-Host "  recovery min/avg/max     : $(if($recoveryStats){('{0:F4} / {1:F4} / {2:F4}' -f $recoveryStats.Min,$recoveryStats.Average,$recoveryStats.Max)}else{'n/a / n/a / n/a'})"}
if($effectiveWeapon -eq 'glock' -or $effectiveWeapon -eq 'mp5' -or $Weapon -eq 'glock' -or $Weapon -eq 'mp5'){Write-Host "  shot growth min/avg/max  : $(if($shotGrowthStats){('{0:F4} / {1:F4} / {2:F4}' -f $shotGrowthStats.Min,$shotGrowthStats.Average,$shotGrowthStats.Max)}else{'n/a / n/a / n/a'})"}
if($effectiveWeapon -eq 'glock' -or $effectiveWeapon -eq 'mp5' -or $Weapon -eq 'glock' -or $Weapon -eq 'mp5'){Write-Host "  pattern index min/avg/max: $(if($patternIndexStats){('{0:N0} / {1:F2} / {2:N0}' -f $patternIndexStats.Min,$patternIndexStats.Average,$patternIndexStats.Max)}else{'n/a / n/a / n/a'})"}
if($effectiveWeapon -eq 'glock' -or $effectiveWeapon -eq 'mp5' -or $Weapon -eq 'glock' -or $Weapon -eq 'mp5'){Write-Host "  pattern x min/avg/max    : $(if($patternOffsetXStats){('{0:F4} / {1:F4} / {2:F4}' -f $patternOffsetXStats.Min,$patternOffsetXStats.Average,$patternOffsetXStats.Max)}else{'n/a / n/a / n/a'})"}
if($effectiveWeapon -eq 'glock' -or $effectiveWeapon -eq 'mp5' -or $Weapon -eq 'glock' -or $Weapon -eq 'mp5'){Write-Host "  pattern y min/avg/max    : $(if($patternOffsetYStats){('{0:F4} / {1:F4} / {2:F4}' -f $patternOffsetYStats.Min,$patternOffsetYStats.Average,$patternOffsetYStats.Max)}else{'n/a / n/a / n/a'})"}
