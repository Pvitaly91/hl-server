...
if(-not $session){[void]$warnings.Add('No session header line was parsed. The analyzer can still summarize event telemetry, but launch metadata is missing.')} ; if($accepted.Count){[void]$obs.Add("Parsed $($accepted.Count) accepted event(s).")}else{[void]$warnings.Add('No accepted weapon telemetry matched the current filter.')}; if($hits.Count){[void]$obs.Add("Parsed $($hits.Count) hit event(s).")}else{[void]$warnings.Add('No hit telemetry matched the current filter.')}; if($movePenaltyAccepted.Count){[void]$obs.Add("Movement-penalty evidence exists ($($movePenaltyAccepted.Count)).")}else{[void]$warnings.Add('No accepted event with move_penalty > 0 matched the current filter.')}; if($effectiveWeapon -eq 'glock' -and -not $rejected.Count){[void]$warnings.Add('No Glock tap-fire rejection evidence was logged for this selection.')}; if($effectiveWeapon -eq 'mp5' -and -not $burstGrowthAccepted.Count){[void]$warnings.Add('No MP5 burst-growth evidence was logged for this selection.')} ; if($dummySpawns.Count){[void]$obs.Add("Dummy lifecycle telemetry was present ($($dummySpawns.Count) spawn, $($dummyRespawns.Count) respawn).")} ; if($dummyHeadshotKills.Count){[void]$obs.Add("Dummy headshot kill evidence exists ($($dummyHeadshotKills.Count)).")}
