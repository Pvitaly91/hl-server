...
if($effectiveWeapon -eq 'glock' -or $effectiveWeapon -eq 'mp5' -or $effectiveWeapon -eq 'shotgun' -or $Weapon -eq 'glock' -or $Weapon -eq 'mp5' -or $Weapon -eq 'shotgun'){Write-Host "  extra spread min/avg/max : $(if($additionalStats){('{0:F4} / {1:F4} / {2:F4}' -f $additionalStats.Min,$additionalStats.Average,$additionalStats.Max)}else{'n/a / n/a / n/a'})"}
if($effectiveWeapon -eq 'glock' -or $effectiveWeapon -eq 'mp5' -or $effectiveWeapon -eq 'shotgun' -or $Weapon -eq 'glock' -or $Weapon -eq 'mp5' -or $Weapon -eq 'shotgun'){Write-Host "  recovery min/avg/max     : $(if($recoveryStats){('{0:F4} / {1:F4} / {2:F4}' -f $recoveryStats.Min,$recoveryStats.Average,$recoveryStats.Max)}else{'n/a / n/a / n/a'})"}
...
if($effectiveWeapon -eq 'glock' -or $effectiveWeapon -eq 'mp5' -or $effectiveWeapon -eq 'shotgun' -or $Weapon -eq 'glock' -or $Weapon -eq 'mp5' -or $Weapon -eq 'shotgun'){Write-Host "  pattern index min/avg/max: $(if($patternIndexStats){('{0:N0} / {1:F2} / {2:N0}' -f $patternIndexStats.Min,$patternIndexStats.Average,$patternIndexStats.Max)}else{'n/a / n/a / n/a'})"}
if($effectiveWeapon -eq 'glock' -or $effectiveWeapon -eq 'mp5' -or $effectiveWeapon -eq 'shotgun' -or $Weapon -eq 'glock' -or $Weapon -eq 'mp5' -or $Weapon -eq 'shotgun'){Write-Host "  pattern x min/avg/max    : $(if($patternOffsetXStats){('{0:F4} / {1:F4} / {2:F4}' -f $patternOffsetXStats.Min,$patternOffsetXStats.Average,$patternOffsetXStats.Max)}else{'n/a / n/a / n/a'})"}
